...
%   valid in multirate
% 
function [NEW_DT, STEP_REJECTED, PERSISTENT] = ec_h211b(Y, DT, ...
    SYS_INDEX, RELTOL, YTYPICAL, MAX_EEST,  PERSISTENT)
...
if any(isnan(Y (:,end))) % the last element is the solution to evaluate
    ...
    if SYS_INDEX > 1  % for later steps, find the error estimate
        ...
        [eEst, ~] = ee_skelboe2000(Y, DT, SYS_INDEX, RELTOL, YTYPICAL);
